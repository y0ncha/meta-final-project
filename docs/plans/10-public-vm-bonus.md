...
- **REQ-003**: Use public application URL format `http://<EC2_PUBLIC_IP>:8080/MeTA/` unless an AWS public DNS name or explicit DNS record is configured.
...
| TASK-024 | Verify from the EC2 VM that `curl -fsS http://localhost:8080/MeTA/ >/dev/null` passes. |  |  |
| TASK-025 | Verify from an external browser or network that `http://<EC2_PUBLIC_IP>:8080/MeTA/` or the public DNS equivalent loads. |  |  |
...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-027 | Define canonical public target variable `PUBLIC_APP_BASE_URL=http://<EC2_PUBLIC_IP>:8080/MeTA/` in `docs/public-app-bonus.md`, or use the AWS public DNS equivalent if DNS is the selected URL. |  |  |
...
- **TEST-009**: On EC2, `curl -fsS http://localhost:8080/MeTA/ >/dev/null` must pass.
- **TEST-010**: From an external browser or network, `http://<EC2_PUBLIC_IP>:8080/MeTA/` or the public DNS equivalent must load.
