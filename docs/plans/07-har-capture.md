...
This plan implements the HAR deliverable required by `final-project.pdf` and `rules/compliance.md`. The result must provide a written scenario description in `docs/har-scenario.md`, a real HAR file captured from the JSP application flow, a repeatable Dockerized Playwright capture command, and validation that the HAR contains requests for the deployed application at `http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/` or `http://tomcat:8080/yonatan-csasznik-yoed-halberstam-niv-levin/`.
...
Follow-up update on 2026-06-11: The HAR capture scenario now matches the full Playwright functional-test journey, including the empty-submit negative validation after the valid submit path. The HAR validator requires both the `nameInput=Yonatan` POST and the empty `nameInput=` POST so the recorded HAR cannot silently drift back to a positive-only scenario.

## 1. Requirements & Constraints

- **REQ-001**: Capture a real HAR file for the JSP application served at `http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/` from the host and `http://tomcat:8080/yonatan-csasznik-yoed-halberstam-niv-levin/` from Docker network `meta`.
- **REQ-002**: Document the exact HAR scenario in `docs/har-scenario.md` using the assignment-facing flow: open the app, click the about link, type text into the name input, click the submit button, observe the success result, reload the app, click submit with an empty input, and observe the validation result.
- **REQ-003**: Align the HAR scenario with existing JSP selectors in `src/main/webapp/index.jsp`: `#aboutLink`, `#nameInput`, `#submitButton`, `#resultMessage`, and `#validationMessage`.
...
- **REQ-007**: Keep the target URL configurable through environment variable `APP_BASE_URL`; default automated Docker capture to `http://tomcat:8080/yonatan-csasznik-yoed-halberstam-niv-levin/` because the capture runner joins Docker network `meta`.
...
- **REQ-009**: Validate that `output/har/meta-functional-flow.har` is valid JSON with top-level object `log`, array `log.entries`, at least one request URL whose pathname starts with `/yonatan-csasznik-yoed-halberstam-niv-levin/`, a valid-submit POST containing `nameInput=Yonatan`, and an empty-submit POST containing `nameInput=`.
...
| TASK-002 | In `scripts/capture-har`, define `PROJECT_ROOT="$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)"`, `PLAYWRIGHT_IMAGE="${PLAYWRIGHT_IMAGE:-mcr.microsoft.com/playwright:v1.60.0-noble}"`, `PLAYWRIGHT_NETWORK="${PLAYWRIGHT_NETWORK:-meta}"`, `APP_BASE_URL="${APP_BASE_URL:-http://tomcat:8080/yonatan-csasznik-yoed-halberstam-niv-levin/}"`, `HAR_PATH="${HAR_PATH:-output/har/meta-functional-flow.har}"`, `LOG_FILE="${LOG_FILE:-output/har/har-capture.log}"`, and `HAR_CONTAINER_NAME="${HAR_CONTAINER_NAME:-meta-har-${BUILD_NUMBER:-local}}"`. | ✅ | 2026-06-10 |
...
| TASK-006 | In `scripts/capture-har`, after a successful capture, print the exact evidence paths `output/har/har-capture.log` and `output/har/meta-functional-flow.har`. | ✅ | 2026-06-10 |
...
| TASK-009 | In `tests/playwright/capture-har.js`, define `appBaseUrl = process.env.APP_BASE_URL || 'http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/'` and `harPath = process.env.HAR_PATH || 'output/har/meta-functional-flow.har'`; create `path.dirname(harPath)` recursively before browser launch. | ✅ | 2026-06-10 |
...
| TASK-011 | In `tests/playwright/capture-har.js`, implement the scenario exactly: `page.goto(appBaseUrl, { waitUntil: 'networkidle' })`, `page.locator('#aboutLink').click()`, assert the URL ends with `#about`, fill `#nameInput` with `Yonatan`, click `#submitButton`, assert `#resultMessage` has text `Hello, Yonatan. MeTA Corporate reviewed your form, opened a committee, and somehow approved it.`, reload the app, click `#submitButton` with an empty input, assert `#validationMessage` has text `Please enter a name before MeTA Corporate schedules a meeting about the empty box.`, close the context, and close the browser. | ✅ | 2026-06-10 |
...
| TASK-016 | In `scripts/validate-har`, require at least one entry whose `request.url` parses as a URL and whose `pathname` starts with `/yonatan-csasznik-yoed-halberstam-niv-levin/`; fail with exit code `66` when no matching request exists. | ✅ | 2026-06-10 |
| TASK-016A | In `scripts/validate-har`, require one valid-submit POST containing `nameInput=Yonatan` and one empty-submit POST containing `nameInput=` so HAR evidence matches the full functional-test flow. | ✅ | 2026-06-11 |
...
| TASK-018 | In `scripts/validate-har`, print a single success line in the exact format `Validated HAR: <path> entries=<count> groupContextRequests=<count>`. | ✅ | 2026-06-10 |
...
| TASK-021 | In `docs/har-scenario.md` section `Target`, document host URL `http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/` and Docker-network URL `http://tomcat:8080/yonatan-csasznik-yoed-halberstam-niv-levin/`. | ✅ | 2026-06-10 |
| TASK-022 | In `docs/har-scenario.md` section `Scenario Steps`, list the full functional flow: open the app, click `About this app`, type `Yonatan` in the `Name` input, click `Submit`, observe `Hello, Yonatan. MeTA Corporate reviewed your form, opened a committee, and somehow approved it.`, reload the app, submit an empty input, and observe `Please enter a name before MeTA Corporate schedules a meeting about the empty box.` | ✅ | 2026-06-10 |
| TASK-023 | In `docs/har-scenario.md` section `Capture Command`, document command `./scripts/capture-har` and override example `APP_BASE_URL=http://host.docker.internal:8080/yonatan-csasznik-yoed-halberstam-niv-levin/ PLAYWRIGHT_NETWORK=bridge ./scripts/capture-har` only if the capture container cannot reach Docker network service name `tomcat`. | ✅ | 2026-06-10 |
| TASK-024 | In `docs/har-scenario.md` section `Evidence Files`, list `output/har/meta-functional-flow.har` and `output/har/har-capture.log`. | ✅ | 2026-06-10 |
...
| TASK-030 | Run `./scripts/deploy-war` and verify it prints `Deployed URL: http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/`. | ✅ | 2026-06-10 |
| TASK-031 | Run `./scripts/capture-har` and verify it exits `0`. | ✅ | 2026-06-10 |
| TASK-032 | Run `./scripts/validate-har output/har/meta-functional-flow.har` and verify it prints `Validated HAR: output/har/meta-functional-flow.har entries=<count> groupContextRequests=<count>`. | ✅ | 2026-06-10 |
| TASK-033 | Run `test -s output/har/meta-functional-flow.har` and verify it exits `0`. | ✅ | 2026-06-10 |
| TASK-034 | Run `test -s output/har/har-capture.log` and verify it exits `0`. | ✅ | 2026-06-10 |
...
- **DEP-004**: `scripts/deploy-war` must deploy `target/yonatan-csasznik-yoed-halberstam-niv-levin.war` to Tomcat before HAR capture runs.
...
- **DEP-006**: Browser selectors in `src/main/webapp/index.jsp` must remain stable: `#aboutLink`, `#nameInput`, `#submitButton`, `#resultMessage`, and `#validationMessage`.
...
- **FILE-008**: `output/har/har-capture.log` - generated ignored command log for local evidence.
...
- **TEST-005**: `./scripts/deploy-war` must deploy the app and print `Deployed URL: http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/`.
...
- **TEST-009**: `test -s output/har/har-capture.log` must pass.
...
- **RISK-004**: The default Docker-network URL `http://tomcat:8080/yonatan-csasznik-yoed-halberstam-niv-levin/` will fail if the disposable capture container does not join network `meta`; validate with `docker compose config --quiet` and rerun after `docker compose up -d tomcat jenkins`.
- **RISK-005**: A HAR captured against `http://tomcat:8080/yonatan-csasznik-yoed-halberstam-niv-levin/` proves the same app flow inside Docker but does not show a browser address bar; final submission still needs the separate Tomcat screenshot with `http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/` visible.
