...
- **REQ-006**: Add a Jenkins preflight stage before disposable test containers that runs `docker --version`, `docker compose version`, `docker info`, and a Docker Pipeline smoke container using the Playwright image.
...
- **REQ-011**: Pass Docker Pipeline args that preserve Jenkins-in-Docker workspace behavior: `--network meta`, `--volumes-from meta-jenkins`, working directory `/workspace/final-project`, and explicit environment values including `APP_BASE_URL=http://tomcat:8080/meta/`.
...
| TASK-016 | Add stage `Docker Pipeline Preflight` before Playwright/Gatling stages; run `docker --version`, `docker compose version`, `docker info`, and a Playwright-image smoke `pwd` through `docker.image(...).inside(...)`. | ✅ | 2026-06-10 |
...
- **TEST-013**: A Jenkins Docker Pipeline smoke run using the Playwright image must prove the container can see `/workspace/final-project` and reach `http://tomcat:8080/meta/`.
