...
This plan deploys the Maven-built JSP WAR `target/yonatan-csasznik-yoed-halberstam-niv-levin.war` into the containerized Apache Tomcat 8.5.x runtime defined in `docker-compose.yml`. The deployment must copy the WAR into the Docker-managed Tomcat `webapps` volume through the running `tomcat` Compose service, serve the application at `http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/`, and produce repeatable local evidence without using host Tomcat.

## 1. Requirements & Constraints

- **REQ-001**: Deploy `target/yonatan-csasznik-yoed-halberstam-niv-levin.war` into the Docker Compose service named `tomcat`.
- **REQ-002**: The deployed WAR filename inside the container must be `/usr/local/tomcat/webapps/yonatan-csasznik-yoed-halberstam-niv-levin.war`.
- **REQ-003**: The expanded Tomcat application directory inside the container must be `/usr/local/tomcat/webapps/yonatan-csasznik-yoed-halberstam-niv-levin`.
- **REQ-004**: The local application URL must be `http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/`.
...
- **REQ-008**: `scripts/deploy-war` must remove stale `/usr/local/tomcat/webapps/MeTA`, `/usr/local/tomcat/webapps/meta`, and `/usr/local/tomcat/webapps/yonatan-csasznik-yoed-halberstam-niv-levin.war` inside the Tomcat container before copying the new WAR.
- **REQ-009**: `scripts/deploy-war` must wait until `curl -fsS http://localhost:${TOMCAT_HOST_PORT:-8080}/yonatan-csasznik-yoed-halberstam-niv-levin/` succeeds or fail with a non-zero exit status after a bounded timeout.
...
- **CON-004**: Do not change the Maven coordinate `mta.devops:meta:1.0.0` or the Maven `<finalName>yonatan-csasznik-yoed-halberstam-niv-levin</finalName>` value created by Plan 03.
...
|------|-------------|-----------|------|
| TASK-001 | Run `rtk read contribution.md` from the repository root and confirm Plan 04 uses containerized Tomcat 8.5.x in Docker, deploys the app under `/yonatan-csasznik-yoed-halberstam-niv-levin/`, and does not depend on host Tomcat or host Jenkins. | ✅ | 2026-06-10 |
...
| TASK-006 | Run `rtk read pom.xml` and confirm `<packaging>war</packaging>` and `<finalName>yonatan-csasznik-yoed-halberstam-niv-levin</finalName>` are present. If either value is missing, stop and report that Plan 03 is incomplete. | ✅ | 2026-06-10 |
...
| TASK-010 | In `scripts/deploy-war`, define `TOMCAT_SERVICE="${TOMCAT_SERVICE:-tomcat}"`, `TOMCAT_CONTEXT="${TOMCAT_CONTEXT:-yonatan-csasznik-yoed-halberstam-niv-levin}"`, `TOMCAT_WEBAPPS_DIR="${TOMCAT_WEBAPPS_DIR:-/usr/local/tomcat/webapps}"`, `TOMCAT_HOST_PORT="${TOMCAT_HOST_PORT:-8080}"`, `DEPLOY_TIMEOUT_SECONDS="${DEPLOY_TIMEOUT_SECONDS:-60}"`, and `WAR_SOURCE="${WAR_SOURCE:-target/yonatan-csasznik-yoed-halberstam-niv-levin.war}"`. | ✅ | 2026-06-10 |
| TASK-011 | In `scripts/deploy-war`, validate `TOMCAT_CONTEXT` equals `yonatan-csasznik-yoed-halberstam-niv-levin`; if it differs, print a grading-clarity error to stderr and exit with status `2`. | ✅ | 2026-06-10 |
...
| TASK-022 | Run `docker compose exec -T tomcat sh -lc 'test -f /usr/local/tomcat/webapps/yonatan-csasznik-yoed-halberstam-niv-levin.war'` and confirm it exits with status `0`. | ✅ | 2026-06-10 |
| TASK-023 | Run `docker compose exec -T tomcat sh -lc 'test -d /usr/local/tomcat/webapps/yonatan-csasznik-yoed-halberstam-niv-levin'` and confirm it exits with status `0`; if expansion is delayed, wait up to `60` seconds before failing. | ✅ | 2026-06-10 |
| TASK-024 | Run `curl -f http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/` from the repository root and confirm it exits with status `0`. | ✅ | 2026-06-10 |
| TASK-025 | Run `curl -s http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/` and confirm the response contains `MeTA`, `About this app`, `nameInput`, and `submitButton`. | ✅ | 2026-06-10 |
| TASK-026 | Capture browser evidence after TASK-024 passes and save it under ignored path `output/screenshots/tomcat-meta-local.png`; verify browser URL state as `http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/` and document any browser-chrome screenshot limitation in `docs/changelog/04-tomcat-container-deployment.changelog.md`. | ✅ | 2026-06-10 |
...
| TASK-028 | In `docs/changelog/04-tomcat-container-deployment.changelog.md`, document files changed, exact tool versions observed, Docker Compose service name `tomcat`, deployed container paths, local URL `http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/`, and all validation commands from TASK-019 through TASK-025. | ✅ | 2026-06-10 |
| TASK-029 | In `docs/changelog/04-tomcat-container-deployment.changelog.md`, document the screenshot evidence path `output/screenshots/tomcat-meta-local.png` and state that generated evidence is ignored by Git. | ✅ | 2026-06-10 |
...
- **ALT-001**: Copy `target/yonatan-csasznik-yoed-halberstam-niv-levin.war` into a host Tomcat installation under `/usr/local/tomcat8/webapps`. Rejected because `rules/compliance.md` explicitly requires containerized Tomcat as the project runtime and forbids relying on host Tomcat.
...
- **ALT-005**: Keep a short brand-only context. Rejected for final evidence because the group-member slug removes a needless assignment-wording risk.
...
- **FILE-001**: `scripts/deploy-war` will be created as the executable deployment entrypoint for building and copying `target/yonatan-csasznik-yoed-halberstam-niv-levin.war` into the Tomcat container.
...
- **FILE-006**: `target/yonatan-csasznik-yoed-halberstam-niv-levin.war` will be generated by Maven and copied into Tomcat, but it must remain untracked.
- **FILE-007**: `output/screenshots/tomcat-meta-local.png` will be generated as ignored evidence and must remain untracked.
...
- **TEST-004**: `test -f target/yonatan-csasznik-yoed-halberstam-niv-levin.war` must exit with status `0` after `./scripts/deploy-war` runs.
...
- **TEST-006**: `docker compose exec -T tomcat sh -lc 'test -f /usr/local/tomcat/webapps/yonatan-csasznik-yoed-halberstam-niv-levin.war'` must exit with status `0`.
- **TEST-007**: `docker compose exec -T tomcat sh -lc 'test -d /usr/local/tomcat/webapps/yonatan-csasznik-yoed-halberstam-niv-levin'` must exit with status `0`.
- **TEST-008**: `curl -f http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/` must exit with status `0`.
- **TEST-009**: `curl -s http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/` must contain `MeTA`, `About this app`, `nameInput`, and `submitButton`.
- **TEST-010**: Browser evidence must be saved at `output/screenshots/tomcat-meta-local.png`, and browser URL state must be verified as `http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/`; if the screenshot API cannot include browser chrome, the limitation must be documented in `docs/changelog/04-tomcat-container-deployment.changelog.md`.
...
- **RISK-003**: Local port `8080` may already be occupied by another process; do not silently change the port because grading evidence expects `http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/`.
- **RISK-004**: Tomcat may need several seconds to expand `yonatan-csasznik-yoed-halberstam-niv-levin.war`; the deployment script must wait before failing.
- **RISK-005**: The former context-name compliance risk is resolved by deploying the group-member slug.
...
- **ASSUMPTION-001**: Plan 03 has already created a valid JSP Maven WAR project that builds `target/yonatan-csasznik-yoed-halberstam-niv-levin.war`.
