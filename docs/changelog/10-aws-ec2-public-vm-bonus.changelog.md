...
- Applied the same restart policy to the existing EC2 `meta-tomcat` container and redeployed the WAR after the instance came back up.
- Updated the public host tracker to use the current Elastic IP `51.84.219.74`.
- Set the live EC2 checkout's uncommitted `.env` to the public `APP_BASE_URL` and `TOMCAT_RESTART_POLICY=unless-stopped` so future EC2 `docker compose up -d tomcat` / `./scripts/deploy-war` runs keep the correct target URL and restart policy after this Compose change is pulled.
...
- EC2 `TOMCAT_RESTART_POLICY=unless-stopped docker compose up -d tomcat` - starts only Tomcat with the EC2 restart policy.
...
- EC2 `.env` update for public `APP_BASE_URL` and `TOMCAT_RESTART_POLICY=unless-stopped` - applied; live `meta-tomcat` still returned `unless-stopped`.
- EC2 `systemctl is-enabled docker` - returned `enabled`.
- EC2 `systemctl is-active docker` - returned `active`.
- Local `curl -I --connect-timeout 10 http://51.84.219.74:8080/yonatan-csasznik-yoed-halberstam-niv-levin/` - returned `HTTP/1.1 200`.
