...
Implemented repeatable HAR capture for the JSP application flow. The capture runs in the official Playwright container, records a full HAR with embedded content for the `/yonatan-csasznik-yoed-halberstam-niv-levin/` app flow, validates the HAR structure and request targets, and documents the scenario for the final submission package.
...
- `scripts/validate-har`: Added a Node validator that checks HAR JSON structure, `/yonatan-csasznik-yoed-halberstam-niv-levin/` request URLs, and response content evidence.
...
- `docs/har-scenario.md`: Added `What This HAR Tests` to explain the expected GET `/yonatan-csasznik-yoed-halberstam-niv-levin/`, same-page `#about` fragment navigation, POST `/yonatan-csasznik-yoed-halberstam-niv-levin/index.jsp`, success response, and out-of-scope evidence items.
...
| `./scripts/deploy-war` | Passed with elevated Docker socket access and printed `Deployed URL: http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/`. |
...
| `test -s output/har/har-capture.log` | Passed. |
...
- Capture log: `output/har/har-capture.log`
...
- Captured request URLs reviewed: `http://tomcat:8080/yonatan-csasznik-yoed-halberstam-niv-levin/` and `http://tomcat:8080/yonatan-csasznik-yoed-halberstam-niv-levin/index.jsp`
...
- The HAR uses Docker-network URL `http://tomcat:8080/yonatan-csasznik-yoed-halberstam-niv-levin/`; the separate final Tomcat screenshot still needs a browser address bar showing `http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/`.
...
## 2026-06-11 Full Functional Flow Follow-Up

- Updated `tests/playwright/capture-har.js` so HAR capture now matches the full Playwright functional-test journey: page load, about-link navigation, valid submit success, app reload, empty-submit validation feedback.
- Updated `scripts/validate-har` to require both the valid-submit POST with `nameInput=Yonatan` and the empty-submit POST with `nameInput=`. This makes the validator fail if the HAR drifts back to a positive-only flow.
- Updated `docs/har-scenario.md` to describe the full scenario and expected network behavior for both form submissions.
- Updated `docs/plans/07-har-capture.md` so Plan 07 reflects the full-flow HAR capture contract.
- Validation: the old HAR failed the updated validator with `HAR file does not contain the empty-submit POST with nameInput=`, then `./scripts/capture-har` regenerated the HAR and `./scripts/validate-har output/har/meta-functional-flow.har` passed with `entries=4` and `groupContextRequests=4`.
