...
- Optional local runtime check `APP_BASE_URL=http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/ ./scripts/run-monitoring-check`: blocked because no service was listening on localhost port 8080 during validation.
...
- Local script runtime evidence still needs to be captured after Tomcat is running at `http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/`.
