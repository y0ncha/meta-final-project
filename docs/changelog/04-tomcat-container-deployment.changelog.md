...
Implemented repeatable deployment of the Maven WAR into the containerized Tomcat runtime. The deployment command is `./scripts/deploy-war`; it builds `target/yonatan-csasznik-yoed-halberstam-niv-levin.war`, ensures the `tomcat` Compose service is running, removes stale `meta` deployment artifacts inside the container, copies the WAR into Tomcat `webapps`, and waits for `http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/`.
...
- Container WAR path: `/usr/local/tomcat/webapps/yonatan-csasznik-yoed-halberstam-niv-levin.war`
- Container expanded app path: `/usr/local/tomcat/webapps/yonatan-csasznik-yoed-halberstam-niv-levin`
- Local app URL: `http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/`
...
- `./scripts/deploy-war`: passed with final output `Deployed URL: http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/`.
...
- `docker compose exec -T tomcat sh -lc 'test -f /usr/local/tomcat/webapps/yonatan-csasznik-yoed-halberstam-niv-levin.war'`: passed.
- `docker compose exec -T tomcat sh -lc 'test -d /usr/local/tomcat/webapps/yonatan-csasznik-yoed-halberstam-niv-levin'`: passed.
- `curl -f http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/`: passed outside the sandbox and returned the JSP HTML.
- `curl -s http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/`: returned HTML containing `MeTA`, `About this app`, `nameInput`, and `submitButton`.
- Browser automation verified the tab URL as `http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/` and page title as `MeTA`.
...
- Screenshot path: `output/screenshots/tomcat-meta-local.png`
...
- Evidence caveat: the saved screenshot captures the rendered browser viewport. The in-app browser screenshot API does not include browser chrome, and the host `screencapture -x output/screenshots/tomcat-meta-local.png` attempt failed with `could not create image from display`, so the address-bar URL is proven by browser automation URL state and HTTP validation instead of appearing inside this screenshot.
- Final submission action: capture a normal manual browser screenshot with `http://localhost:8080/yonatan-csasznik-yoed-halberstam-niv-levin/` visible in the address bar before sending the final assignment package.
...
- Clarified that `output/screenshots/tomcat-meta-local.png` is supplemental evidence and that the final submission still needs a manual address-bar screenshot.
