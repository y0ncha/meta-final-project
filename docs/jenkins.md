...
Recommended SLA-oriented evidence values are `GATLING_LOAD_USERS=250`, `GATLING_STRESS_START_USERS=250`, `GATLING_STRESS_TARGET_USERS=475`, and max-limit range `450-550` users/sec with `25` users/sec steps. The hard gate is `KO=0`; the recommended load/stress latency SLA is global `p95 < 2000 ms`. These values are derived from local build `#12` (`475` pass / `500` fail) and public build `#13` (`525` pass / `550` fail). If the Jenkinsfile defaults are not changed, set the matching environment values before collecting refreshed load/stress SLA evidence.
