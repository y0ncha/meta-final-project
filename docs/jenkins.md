...
6. `Gatling Max Limit`: Runs only when `scripts/run-gatling-max-limit` exists and build parameter `RUN_GATLING_TESTS=true`. Jenkins starts the Gatling image through Docker Pipeline, verifies the Gatling container workspace, and calls `GATLING_DOCKER_PIPELINE=1 GATLING_RUN_TYPE=max-limit ./scripts/run-gatling-max-limit`. The wrapper runs bounded full discovery attempts and stops when a normalized threshold-failure report is produced or the configured attempt count is exhausted. `GATLING_CONSOLE_MODE=summary` keeps passing levels out of the console, prints Gatling metrics when a failing level is reached, then prints the wrapper boundary summary with the highest passing tested virtual-user level. Complete logs stay archived under `output/gatling/max-limit/`. This keeps disruptive Gatling evidence collection out of ordinary CI/CD runs while still making it Jenkins-runnable for final evidence capture.
7. `Gatling Load Test`: Runs only when `scripts/run-gatling-load-5m` exists and build parameter `RUN_GATLING_TESTS=true`. Jenkins uses that wrapper as the existence gate, then starts the Gatling image through Docker Pipeline, verifies the Gatling container workspace and shared runner script, and calls `GATLING_DOCKER_PIPELINE=1 GATLING_RUN_TYPE=load-5m ./scripts/run-gatling-container`. This is the required five-minute Gatling load test. The console mode parameter controls whether Jenkins prints the complete Gatling log or only summary lines.
8. `Gatling Stress Test`: Runs only when `scripts/run-gatling-stress-5m` exists and build parameter `RUN_GATLING_TESTS=true`. Jenkins uses that wrapper as the existence gate, then starts the Gatling image through Docker Pipeline, verifies the Gatling container workspace and shared runner script, and calls `GATLING_DOCKER_PIPELINE=1 GATLING_RUN_TYPE=stress-5m ./scripts/run-gatling-container`. This is the required five-minute Gatling stress test. The complete Gatling output is preserved in `output/gatling/stress-5m/stress-5m-run.log` even when summary console mode is selected.
