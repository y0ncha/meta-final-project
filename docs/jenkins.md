...
For max-limit submission evidence, Jenkins uses a users/sec generator sweep to produce the Gatling report, but the max-limit value to write in the submission is the active-users count at the selected `Number of responses per second` tooltip where `KO=0`. The refreshed local evidence in `builds/max-limit-local/` supports `2340 active users`, `1399 OK`, `0 KO`; the refreshed public evidence in `builds/max-limit-public/` supports `6718 active users`, `301 OK`, `0 KO`.
