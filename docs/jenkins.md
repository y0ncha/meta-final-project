...
6. `Gatling Max Limit`: Runs only when `scripts/run-gatling-max-limit` exists and build parameter `RUN_GATLING_TESTS=true`. Jenkins starts the Gatling image through Docker Pipeline, verifies the Gatling container workspace, and calls `GATLING_DOCKER_PIPELINE=1 GATLING_RUN_TYPE=max-limit ./scripts/run-gatling-max-limit`. The wrapper runs one bounded staircase simulation from `GATLING_MAX_BASE_USERS` through `GATLING_MAX_LIMIT_USERS` and keeps the normalized staircase report even when Gatling exits non-zero after assertion evaluation. `GATLING_CONSOLE_MODE=summary` keeps the console compact, prints Gatling metrics, then prints the wrapper staircase summary and KO cutoff rule. Complete logs stay archived under `output/gatling/max-limit/`. This keeps disruptive Gatling evidence collection out of ordinary CI/CD runs while still making it Jenkins-runnable for final evidence capture.
